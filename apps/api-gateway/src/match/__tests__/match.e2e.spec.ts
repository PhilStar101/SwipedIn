...
describe('Match', () => {
  ...
  describe('createMatch', () => {
    describe('createHirerMatch', () => {
      it('should create match for the Hirer', async () => {
        const inserted = await insertMockProfiles();
        const createMatchDto: CreateMatchDto = {
          hirer: inserted.hirers[0],
          employee: inserted.employees[0],
        };

        const response = await request(httpServer)
          .post('/api/match/hirer')
          .send(createMatchDto);
        ...
        const hirer = await dbConnection
          .collection('matches')
          .findOne({ fromRole: Role.Hirer });
        expect(hirer).toBeTruthy();
      });
    });

    describe('createEmployeeMatch', () => {
      it('should create match for the Employee', async () => {
        const inserted = await insertMockProfiles();

        const createMatchDto: CreateMatchDto = {
          hirer: inserted.hirers[0],
          employee: inserted.employees[1],
        };

        const response = await request(httpServer)
          .post('/api/match/employee')
          .send(createMatchDto);
        ...
        const hirer = await dbConnection
          .collection('matches')
          .findOne({ fromRole: Role.Employee });
        expect(hirer).toBeTruthy();
      });
    });
  });
  ...
  describe('getMatch', () => {
    ...
    describe('getHirerMatch', () => {
      ...
      it('should get 1 match(es) when 2 employe(es) accepts 1 hirer(s) and 1 hirer accepts 1 employee(s)', async () => {
        const insertMatch = await insertMatchFactory();
        await insertMatch(Role.Employee, 0, 0);
        await insertMatch(Role.Employee, 0, 1);
        const { hirerId } = await insertMatch(Role.Hirer, 0, 0);
      ...
      it('should get 1 match(es) when 3 employe(es) accepts 1 hirer(s) and 1 hirer accepts 1 employee(s)', async () => {
        const insertMatch = await insertMatchFactory();
        await insertMatch(Role.Employee, 0, 0);
        await insertMatch(Role.Employee, 0, 1);
        await insertMatch(Role.Employee, 0, 2);
        const { hirerId } = await insertMatch(Role.Hirer, 0, 0);
      ...
      it('should get 2 match(es) when 2 employe(es) accepts 1 hirer(s) and 1 hirer accepts 2 employee(s)', async () => {
        const insertMatch = await insertMatchFactory();
        await insertMatch(Role.Employee, 0, 0);
        await insertMatch(Role.Employee, 0, 1);
        const { hirerId } = await insertMatch(Role.Hirer, 0, 0);
        await insertMatch(Role.Hirer, 0, 1);
      ...
      it('should get 2 match(es) when 3 employe(es) accepts 1 hirer(s) and 1 hirer accepts 2 employee(s)', async () => {
        const insertMatch = await insertMatchFactory();
        await insertMatch(Role.Employee, 0, 0);
        await insertMatch(Role.Employee, 0, 1);
        await insertMatch(Role.Employee, 0, 2);
      ...
      it('should get 3 match(es) when 3 employe(es) accepts 1 hirer(s) and 1 hirer accepts 3 employee(s)', async () => {
        const insertMatch = await insertMatchFactory();
        await insertMatch(Role.Employee, 0, 0);
        await insertMatch(Role.Employee, 0, 1);
        await insertMatch(Role.Employee, 0, 2);
        const { hirerId } = await insertMatch(Role.Hirer, 0, 0);
        await insertMatch(Role.Hirer, 0, 1);
        await insertMatch(Role.Hirer, 0, 2);
